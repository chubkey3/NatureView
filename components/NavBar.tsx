    ...
    return (
       <Flex position={'fixed'} bottom={0} bgColor={'white'} w={'100vw'} h={['60px', '80px']} zIndex={999} overflow={'hidden'} justifyContent={'center'}>
            <Flex w={'100%'} maxW={'600px'} h={'100%'} justifyContent={'space-around'} alignItems={'center'}>
                ...
                <IconButton  as={RiSlideshow2Line}  _hover={{bgColor: 'green.300'}} bgColor={router.pathname === "/slideshow" ? 'green.500' : 'transparent'} color={router.pathname === "/slideshow" ? 'white' : 'black'} size={'lg'} p={1.5} fontSize={'30px'} aria-label='slideshow' onClick={() => process.env.NODE_ENV === 'production' ? toast({title: 'Coming Soon!', description: 'Feature in progress! Come back later to try it out!', status: 'info', duration: 3000, isClosable: true, position: 'top'}) : redirect("/slideshow")}/>                
